...
public class SoilInfoGet{
    ...
    private void process() throws IOException {
    	...
    	while(percentInfo.contains("100.0%")){
        	...
        	tempCounty = tempCounty.trim();
        	int indexIdBegin = percentInfoPiece.indexOf("Percent of AOI")+16;
        	String temp = percentInfoPiece.substring(indexIdBegin,indexIdBegin+25);
        	...
        	while(!tempId.contains("Subtot") && !tempId.contains("Totals")){
            	...
            	percents[indexArray] = tempPercent/100;
            	indexArray++;
            	temp = percentInfoPiece.substring(indexPercentEnd+2,indexPercentEnd+25);
            	st = new StringTokenizer(temp);
            	tempId = st.nextToken()+'—'+st.nextToken();
            	if(tempId.contains("Custom")){
            		int nextTableFirstIdIndex = percentInfoPiece.indexOf("Percent of AOI")+16;
                	String tempLine = percentInfoPiece.substring(nextTableFirstIdIndex,nextTableFirstIdIndex+25);
    	...
    	for(int j=0;j<indexArray;j++){
    		System.out.println("county: " + counties[j] + " id: "+ ids[j]+ " percent: "+df.format(percents[j]));
    	}*/
    	
    	//System.out.println(valueInfo);
    	/*the next part is to analyze and catch value data*/
    	//calculate valid value information string
    	...
    	int countyCount = 0;//traversal all counties
    	int itemCount = 0; //traversal all items
    	...
    	//piece by piece to analyze and catch value data
    	while(countyCount<numOfCounties){
    		String valueInfoPiece;// contain one county string piece
    		int indexOfCounty = 0;
    		...
    		int iCount = 0;//traversal items of one county
    		//System.out.println("numOfItems = "+numOfItems);
    		while(iCount<numOfItems){
    			String littlePiece; //contain one item piece of one county
    			float high = 0;
    			float veryHigh = 0;
    			float depth = 0;
    			int indexOfId = 0;
    			...
    			if(iCount+1 < numOfItems){
    				...
    				//System.out.println("indexOfId = "+ indexOfId+", totalLength = "+valueInfoPiece.length());
    			...
    			if(littlePiece.contains(key)){
    				...
    				st.nextToken(); String tempVeryHigh = st.nextToken();
    				veryHigh = Float.parseFloat(isNumeric(tempVeryHigh)?tempVeryHigh:"0.0");
    			}else{
    				high = veryHigh = 0;
    			}
    			...
    			if(littlePiece.contains(key)){
    				...
    				st.nextToken();//"to"
    				value = st.nextToken();//for "About 30 to 47 inches" case
    				if(isNumeric(value)){
    					depth = (depth+Float.parseFloat(value))/2;
    				}
    			}else{
    				depth = 0;
    			}
